...
for muActual in muRange
    ...
    for _ in 1:N
        ...
        pValT = 2.0*(ccdf(TDist(n-1), abs(tStatT)))
        
        xPositive = sum(data .> mu0)
        uStat = max(xPositive, n-xPositive)
        pValSign = 2.0*(pdf(Binomial(n,0.5), uStat))
...
plot!(muRange, powerU, c=:red, label="Sign test", xlims=(51,55), ylims=(0,1), xlabel="Different values of muActual",ylabel="Proportion of times H0 rejected", legend=:bottomleft)
